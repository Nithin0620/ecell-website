...
const EventCard = ({ event, index }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative md:w-1/2 h-56 sm:h-64 md:h-[480px] overflow-hidden">
          <img
            src={event.image}
            alt={event.title}
        ...
        <div className="relative md:w-1/2 p-5 sm:p-7 md:p-12 flex flex-col justify-center bg-gradient-to-br from-emerald-50/80 via-cyan-50/70 to-sky-50/80 backdrop-blur-2xl border border-white/40">
          <div className="absolute inset-0 flex items-center justify-center gap-4 lg:gap-6 opacity-100 transition-all duration-500 md:group-hover:opacity-0 pointer-events-none z-0">
            {displayIcons.map((icon, iconIndex) => (
              <div
                ...
                className={`w-12 h-12 lg:w-16 lg:h-16 rounded-2xl ${icon.bg} ${icon.text} flex items-center justify-center shadow-lg`}
              >
                <svg viewBox="0 0 24 24" className="w-6 h-6 lg:w-8 lg:h-8" fill="currentColor" aria-hidden="true">
                  <path d={icon.path} />
                </svg>
              </div>
            ))}
          ...
          <div className="flex flex-col gap-4 relative z-10">
            <div className="transition-all duration-500 opacity-100 md:group-hover:opacity-0 md:group-hover:-translate-y-2">
              <h3 className="font-oswald text-2xl sm:text-3xl md:text-4xl font-bold text-primary-dark">
                {event.title}
              </h3>
            </div>
            <div className="transition-all duration-500 opacity-100 translate-y-0 md:opacity-0 md:translate-y-2 md:group-hover:opacity-100 md:group-hover:translate-y-0">
              <p className="font-merriweather text-sm sm:text-base text-slate-800 leading-relaxed">
                {event.description}
              </p>
            </div>
          </div>

          <div className="mt-5 sm:mt-6 flex flex-wrap items-center gap-3 sm:gap-4 relative z-10">
            <div className="flex items-center gap-2 bg-white/70 px-3 sm:px-4 py-2 rounded-full border border-sky-200/70 shadow-lg shadow-sky-200/30 opacity-100 translate-y-0 transition-all duration-500 md:opacity-0 md:translate-y-4 md:delay-100 md:group-hover:opacity-100 md:group-hover:translate-y-0">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-main-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <span className="font-merriweather text-xs sm:text-sm text-slate-900">
                {displayedDate}
              </span>
            </div>

            <div className="flex items-center gap-2 bg-white/70 px-3 sm:px-4 py-2 rounded-full border border-emerald-200/70 shadow-lg shadow-emerald-200/30 opacity-100 translate-y-0 transition-all duration-500 md:opacity-0 md:translate-y-4 md:delay-200 md:group-hover:opacity-100 md:group-hover:translate-y-0">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-main-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
              <span className="font-merriweather text-xs sm:text-sm text-slate-900 font-semibold">
